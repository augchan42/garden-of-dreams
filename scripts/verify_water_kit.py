"""Verify each exported water variant's actual geometry, LOD and connectors."""
from pathlib import Path
import json, struct
R=Path(__file__).resolve().parents[1]
expected={v:({'south':(0,0,3),'north':(0,0,-3)} if v.endswith('bridge') else {}) for v in ['stream','pond','embankment','lotus','wood_bridge','stone_bridge']}
report={}
for variant,ports in expected.items():
 totals=[];collisions=[]
 for suffix in ['', '_LOD1']:
  p=R/'export/kits/water'/f'KIT_water_{variant}{suffix}.glb'
  ...
  for node in d['nodes']:
   ...
   if name.startswith('PORT_'):
    ...
    assert node.get('extras',{}).get('connection')=='water_bridge_1.8m'
  ...
  assert len(cols)==(3 if variant.endswith('bridge') else (1 if variant=='embankment' else 0)),(p,len(cols))
...
(R/'export/kits/water/validation.json').write_text(json.dumps(report,indent=2))
print('WATER_EXPORT_PASS',json.dumps(report))
