...
func capture(label:String) -> void:
 ...
 if root.get_texture().get_image().save_png("res://../docs/reference/route-"+label+".png")!=OK:
  push_error("Red courtyard reference could not be saved")
  quit(1)
func run() -> void:
 ...
 var prefix="mobile-red-court" if mobile else "red-court"
 await capture(prefix)
 route.execute_command("doors")
 await capture(prefix+"-doors")
 route.execute_command("leaves")
 await capture(prefix+"-leaves")
 print("RED_COURT_VIEWS_SAVED ",prefix)
 quit(0)
